...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key,Value> {
    ...
    private static class Entry {
        ...
        public Entry(Comparable key, Object val, Node child, String filePath) {
            this.key = key;
            this.val = val;
            this.child = child;
            ...
            if (nullCount != 2) {
                throw new IllegalArgumentException ("An entry can only have a value, child, or filepath");
            }
        }
    ...
    public Value get(Key k) {
        ...
        if(entry != null) {
            if (this.disk.contains(k)) {
                try {
                    Value val = this.bringBackFromDisk(k);
                    if (val instanceof Document) {
                        ((Document) val).setLastUseTime(System.nanoTime());
                    }
                    this.pm.delete(k);
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    public Value put(Key k, Value v) {
        //need to add delete
        Value valueToReturn = this.get(k);
        if (k == null) {
        ...
        newRoot.entries[0] = new Entry(this.root.entries[0].key, null, this.root, null);
        newRoot.entries[1] = new Entry(newNode.entries[0].key, null, newNode, null);
        this.root = newRoot;
    ...
    private Node put(Node currentNode, Key key, Value val, int height) {
        int j;
        Entry newEntry = new Entry(key, val, null, null);
    ...
    public void moveToDisk(Key k) throws Exception {
        if (this.pm == null) {
            throw new IllegalStateException("Btree can't move to disk if no pm has been set. ");
        }
        Entry entry = this.get(this.root, k, this.height);
        Value value = (Value)entry.val;
        if (this.disk.contains(k)) {
            this.pm.delete(k);
            return;
        }
        this.pm.serialize(k, value);
        String filePath = k.toString();
        this.disk.add(k);
        entry.val = null;
    ...
    private Value bringBackFromDisk(Key k) throws IOException {
        //removes from the disk
        Entry entry = this.get(this.root, k, this.height);
        Value val = (Value) this.pm.deserialize(k);
        this.disk.remove(k);
        entry.filePath = null;
        entry.val = val;
        return (Value) entry.val;
